window.dataLovers = {
  filterCountry: function filterCountry(country, indicatorsPerceptionCult) {
    ...
    const indicatorsCountry = dataCountry.indicators.filter(function(
      indicator
    ) {
      const indicatorCode = indicator.indicatorCode;
      const indicatorCodePerception = indicatorsPerceptionCult.map(
        ind => ind.indicatorCode
      );
  ...
  showIndicatorNamesCountry: function showIndicatorNamesCountry(
    indicators,
    parentElement
  ) {
  ...
  filterIndicatorYear: function filterIndicatorYear(
    code,
    country,
    indicatorsPerceptionCult
  ) {
    const countryInd = window.dataLovers.filterCountry(
      country,
      indicatorsPerceptionCult
    );
    const dataYear = countryInd.filter(
      indicator => indicator.indicatorCode == code
    );
